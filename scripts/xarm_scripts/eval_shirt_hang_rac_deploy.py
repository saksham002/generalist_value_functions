...
class EvalArgs:
    ...
    record: bool = False  # If True, prompt to save episode data as HDF5 after each episode
    save_data_dir: str = "data/openpi_shirt_hang_correction"  # Directory to save HDF5 files
...
def rollout_episode(
...
) -> tuple[float, float, dict[str, Path], dict, dict]:
    ...
    Returns:
        ...
        recorded_data: Dict of recorded HDF5 data keyed by dataset path
    """
    ...
    # Data recording (for HDF5 saving)
    # We store all state keys, all images, actions, rewards, etc.
    recorded_images = {}   # cam_name -> list of JPEG-encoded buffers
    recorded_states = {}   # state_key -> list of arrays
    recorded_timestamps = []  # wall-clock time per step
    recorded_actions = []
    recorded_rewards = []
    recorded_dones = []
    recorded_truncateds = []
    recorded_intervene_actions = []
    ...
    while not (terminated or truncated):
        ...
        # Record data (for HDF5 saving)
        if args.record:
            # Store all images as JPEG bytes
            for cam_name, cam_frames in obs["images"].items():
                frame = cam_frames[-1] if cam_frames.ndim == 4 else cam_frames
                _, encoded = cv2.imencode(".jpg", frame, encode_param)
                recorded_images.setdefault(cam_name, []).append(encoded)

            # Store all state fields
            for state_key, state_val in obs["state"].items():
                val = state_val[-1] if state_val.ndim > 1 else state_val
                recorded_states.setdefault(state_key, []).append(val.astype(np.float32))

            recorded_timestamps.append(time.time())
            recorded_actions.append(action.copy())
            recorded_rewards.append(reward)
            recorded_dones.append(terminated)
            recorded_truncateds.append(truncated)
            recorded_intervene_actions.append(info.get("intervene_action", np.zeros_like(action)))
    ...
    recorded_data = {
        ...
        "dones": recorded_dones,
        "truncateds": recorded_truncateds,
        "intervene_actions": recorded_intervene_actions,
    } if args.record else {}
...
def evaluate_checkpoint(
    ...
    while episodes_progress_bar.n < episodes_progress_bar.total:
        try:
            ...
            if args.record and recorded_data and recorded_data.get("actions"):
                is_save_data = input("Finished episode. Save data? (y/n): ")
                if "y" in is_save_data.lower():
                    task_name = args.config
                    file_name = f"episode_{episode_idx}_{time.strftime('%Y%m%d_%H%M%S')}.hdf5"
                    save_dir = Path(args.save_data_dir)
                    save_dir.mkdir(parents=True, exist_ok=True)
                    file_path = save_dir / file_name

                    logging.info(
                        f"Saving HDF5 to {file_path}, with {episode_metrics['intervention_step_counter']} steps of intervention"
                    )
                    with h5py.File(file_path, "w") as f:
                        # Metadata
                        f["metadata/task"] = "hang the shirt"
                        f["metadata/config"] = task_name

                        # Timestamps
                        f["obses/state/timestamp"] = np.array(recorded_data["timestamps"], dtype=np.float64)

                        # All state fields
                        for state_key, state_list in recorded_data["states"].items():
                            f[f"obses/state/{state_key}"] = np.stack(state_list)

                        # All images — variable-length JPEG bytes
                        dt = h5py.vlen_dtype(np.dtype("uint8"))
                        for cam_name, img_list in recorded_data["images"].items():
                            ds = f.create_dataset(f"obses/images/{cam_name}", (len(img_list),), dtype=dt)
                            for i, jpeg_buf in enumerate(img_list):
                                ds[i] = np.frombuffer(jpeg_buf, dtype=np.uint8)

                        # Actions, rewards, dones, interventions
                        f["actions"] = np.stack(recorded_data["actions"])
                        f["rewards"] = np.array(recorded_data["rewards"], dtype=np.float32)
                        f["dones"] = np.array(recorded_data["dones"], dtype=bool)
                        f["truncateds"] = np.array(recorded_data["truncateds"], dtype=bool)
                        f["intervene_actions"] = np.stack(recorded_data["intervene_actions"])
