...
if [ ! -f "scripts/eval_shirt_hang_rac_deploy.py" ]; then
    echo -e "${RED}ERROR: scripts/eval_shirt_hang_rac_deploy.py not found!${NC}"
    echo -e "${YELLOW}Looking in: ${OPENPI_ROOT}/scripts/eval_shirt_hang_rac_deploy.py${NC}"
    exit 1
fi
...
uv run scripts/eval_shirt_hang_rac_deploy.py \
    "${CONFIG}" \
