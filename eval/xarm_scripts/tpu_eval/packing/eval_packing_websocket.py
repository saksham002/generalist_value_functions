...
Usage:
    ...
    ./eval/xarm_scripts/tpu_eval/packing/serve_policy_packing.sh
...
class Args:
    ...
    """Directory to save debug images when --debug is set. Defaults to eval/xarm_scripts/tpu_eval/packing/debug/."""
    ...
    to eval/xarm_scripts/tpu_eval/packing/videos/episode_<n>.mp4 at FPS = control_freq / query_freq."""
    ...
    --task-description (the prompt the policy is conditioned on). Overridden per episode by the
    task loaded from `tasks_file` when that file is present."""

    tasks_file: str = ""
    """Path to the packing-tasks JSON. Empty string defaults to packing_tasks.json next to this
    script. When the file exists, each episode loads the task at its own episode_idx (tasks are
    flattened across the category keys in order: small_medium 0-7, small_large 8-15,
    medium_large 16-23) and uses it as the prompt sent to the policy (overriding
    `task_description`)."""
...
def load_packing_task(tasks_file: str, task_index: int) -> tuple[str, dict]:
    """Load the task at `task_index` from the packing-tasks JSON.
    ...
    with open(tasks_file) as f:
        data = json.load(f)
    tasks = [entry for category in ("small_medium", "small_large", "medium_large") for entry in data.get(category, [])]
    if not tasks:
        raise ValueError(f"No tasks found in {tasks_file}; expected category keys small_medium/small_large/medium_large.")
    if not 0 <= task_index < len(tasks):
        raise IndexError(f"task_index {task_index} out of range for {tasks_file} ({len(tasks)} tasks; valid 0..{len(tasks) - 1}).")
...
def main(args: Args) -> None:
    ...
    tasks_file = args.tasks_file or os.path.join(os.path.dirname(os.path.abspath(__file__)), "packing_tasks.json")
